...
async function logManualReconciliation(reference: string, schoolId: string, payload: any, error: any, dbIndex?: number) {
    try {
    ...
    try {
        ...
        try {
            if (isNewSchool) {
                ...
                batch.set(db.collection('meta').doc('database_info'), {
                ...
                batch2.set(schoolRef.collection('config').doc('metadata_bundle'), {
            ...
            const qVariants = schoolsRef
                ...
                .where(admin.firestore.FieldPath.documentId(), '<=', `${f`);
