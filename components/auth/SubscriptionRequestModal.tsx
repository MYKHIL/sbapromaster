...
const SubscriptionRequestModal: React.FC<SubscriptionRequestModalProps> = ({ isOpen, onClose, initialSchoolName }) => {
    const [selectedTier, setSelectedTier] = useState(SUBSCRIPTION_TIERS[1].name); // Default to Basic
    ...
    useEffect(() => {
        if (isOpen && initialSchoolName) {
            setSearchTerm(initialSchoolName);
            // Try to find the school if we have an initial name
            findSchool(initialSchoolName);
        }
    }, [isOpen, initialSchoolName]);

    const findSchool = async (name: string) => {
        if (name.length < 3) return;
        setIsSearching(true);
        const list = await getSchoolList(name);
        setSchools(list);
        setIsSearching(false);
        if (list.length > 0) setShowResults(true);
    };

    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            if (searchTerm && searchTerm !== selectedSchool?.displayName) {
    ...
    const handleSend = () => {
        const tier = SUBSCRIPTION_TIERS.find(t => t.name === selectedTier);
                ...
                <div className="p-6 space-y-6 overflow-y-auto">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            {showResults && schools.length > 0 && !selectedSchool && (
                                <div className="absolute left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-10 max-h-48 overflow-y-auto">
                                    {schools.map((school) => (
                                        <button
                                            key={school.docId}
                                            onClick={() => {
                        ...
                        {selectedSchool && (
                            <div className="flex items-center gap-2 text-green-600 text-sm mt-1 animate-fadeIn">
                                ...
                                <span>School verified and selected</span>
                            </div>
                        )}
                    </div>
