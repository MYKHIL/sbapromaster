                            ...
                            <tbody>
                                {filteredSubjects.length > 0 ? (
                                    filteredSubjects.map((subject, index) => {
                                        ...
                                        return (
                                            <tr key={subject.id} className={`border-b transition-colors ${isDirtyRow ? 'bg-amber-50 hover:bg-amber-100' : 'hover:bg-gray-50'}`}>
                ...
                <div className="lg:hidden space-y-4">
                    {filteredSubjects.length > 0 ? (
                        filteredSubjects.map((subject, index) => {
                            const isDirtyRow = isItemDirty('subjects', subject.id);
                            return (
                                <div key={subject.id} className={`p-4 rounded-xl shadow-md border transition-colors flex justify-between items-center ${isDirtyRow ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-200'}`}>
