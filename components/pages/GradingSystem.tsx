            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    {isAdmin && (
                        <button
                            onClick={() => saveGrades()}
                            disabled={!isDirty('grades') || isSyncing || !isOnline}
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-lg transition-all shadow-md ${(!isDirty('grades') || isSyncing || !isOnline)
                                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                    : 'bg-blue-600 text-white hover:bg-blue-700 hover:shadow-lg'
                                }`}
                            title={!isOnline ? "You are offline" : !isDirty('grades') ? "No unsaved changes" : "Save grading system to the cloud"}
                        >
                            {isSyncing ? (
                                <>
