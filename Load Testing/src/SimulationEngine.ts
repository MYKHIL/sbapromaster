...
import { loggedGetDocs, loggedSetDoc } from '../../services/firebaseService';
import {
    Student,
    Subject,
    Class,
...
export class SimulationEngine {
    ...
    async setupDummySchool(schoolName: string) {
        ...
        try {
            await loggedSetDoc(doc(db, "schools", this.docId), schoolData, undefined, `SimulationEngine/createSchool/${this.docId}`);

            // Create subscription to satisfy security rules
            const baseId = this.docId.split('_')[0];
            await loggedSetDoc(doc(db, "subscriptions", baseId), {
                expiryDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000), // 1 year from now
                activationHash: "c93a215026f36ac783bcac8ba5e4bbea1c3cdb6c79d3824f9712143c44dbb0f3"
            }, undefined, `SimulationEngine/createSubscription/${baseId}`);
    ...
    private async simulateUserAction(userId: number, intensity: number) {
        while (this.isRunning) {
            try {
                ...
                const snap = await loggedGetDocs(q, `SimulationEngine/fetchStudents/${this.docId}`);
                ...
                if (snap.docs.length > 0) {
                    ...
                    await loggedSetDoc(sDoc, score, { merge: true }, `SimulationEngine/saveScore/${scoreId}`);
